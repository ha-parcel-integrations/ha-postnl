...
def _polling_entry(hass, options):
    from pytest_homeassistant_custom_component.common import MockConfigEntry

    entry = MockConfigEntry(
        domain=DOMAIN,
        unique_id=_POLLING_ENTRY_DATA["username"],
        data=_POLLING_ENTRY_DATA,
        options=options,
    )
    entry.add_to_hass(hass)
    entry.runtime_data = MagicMock()
    entry.runtime_data.auth.check_and_refresh_token = AsyncMock()
    entry.runtime_data.auth.access_token = "tok"
    return entry


def _empty_shipments() -> MagicMock:
...
def test_map_observation_status_live_catalogue_2026_09_08():
    """Codes reported by a user (issue #20)."""
    # "Sorry, bezorgmoment is bijgewerkt" — a delay, variant of G01/G05/T04.
