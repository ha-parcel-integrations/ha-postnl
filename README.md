...
- **Normal (every 45 minutes)** — for everything else still on its way,
  including mail. This is also the cadence used the rest of the time, so a
  new shipment or letter that shows up in your account is still discovered
  without you having to do anything.
- A small, fixed per-install offset is added on top, so not every PostNL
